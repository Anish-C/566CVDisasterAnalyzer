...
Used to extract building footprints from satellite imagery
"""
...
class Up(nn.Module):
    ...
    def forward(self, x1, x2):
        x1 = self.up(x1)
        # Handle size mismatch between skip connection and upsampled feature
...
class UNet(nn.Module):
    ...
    Args:
        n_channels: Number of input channels (3 for RGB, 6 for pre+post disaster images)
...
def get_unet(n_channels=3, n_classes=1, pretrained=False):
    """
...
if __name__ == "__main__":
    # Test the model
    model = get_unet(n_channels=3, n_classes=1)
